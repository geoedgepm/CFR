...
// Incident report page initialization
$scope.init = function init()
{

  $scope.disaster_er_msg = false;
  $scope.location_er_msg = false;
  $scope.contact_er_msg = false;
  ...
  // Map click event to palce marker
...
if($scope.selectedDisaster == "-1")
 $scope.disaster_er_msg = true;
if(!latitude)
 $scope.location_er_msg = true;
if(!is_number_valid)
  $scope.contact_er_msg = true;
...
for (i = 0; i <= data.results[0].address_components.length - 1; i++) {
    ...
    for (f = 0; f <= data.results[0].address_components[i].types.length - 1; f++) {
        ...
        if (angular.equals(data.results[0].address_components[i].types[f], "administrative_area_level_1")) {
            ...
            if($scope.getProvince)
              $scope.getProvince = $scope.getProvince.replace(/([A-Z])/g, ' $1').trim();
        }
    }
  }
...
// Fetch close end questions
$scope.select_questions = function select_questions(language)
{
    $http(
    {method: "POST",
    ...
    contentType: "application/json; charset=utf-8",
    headers: {"X-CSRFToken": $cookies["csrftoken"]},
    data:JSON.stringify({"disaster": $scope.selectedDisaster, "language": language
    })
    }
    ).success(function(data) {
...
// Dynamic form for close end questions
incidentApp.directive("rnStepper", ["$document", "$compile" ,function($document, $compile) {
    ...
    return {
        restrict: "AE",
         ...
         formData: "=ngModel",
         language_select_dropdown: '=languageSelectDropdown'
        },
        ...
        link: function ($scope, element, attrs, ngCtrl) {
              ...
              angular.forEach($scope.formTemplate, function(value, key) {
                        ...
                        +"<option value=''>"+attrs.languageSelectDropdown.replace(/^'(.*)'$/, '$1');
                        ...
                        angular.forEach(value.options, function (option) {
                        element_string =
                        "<label class='col-md-6 checkbox-inline'><input type='checkbox' value="+option.value+">"+option.label+"";
                        newDirective = angular.element(element_string);
                        ...
                        +"<input type='radio' name="+value.model+" value="+option.label+">"+option.label+""
                        +"</label>"
                        ;
...
// Initial page load for Incident validation
$scope.init = function init()
{
  map = new ol.Map({
    layers: [
